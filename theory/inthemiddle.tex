...
\begin{algorithm}[tbp]
	...
	\Repeat{\(\argmax[x_i]{\vcomp_i(x_i)}\) did not change for any \(i\)}{
		\For{\(j=1,\dotsc,m\)}{
			\(\bar{\vcomp^j}, s^j \leftarrow \text{\UpdateConstraint{\(\ccomp_j\), \(\bar{\vcomp}^j\), \(s^j\)}}\)
		}
	}
	% [todo] - fix the strange empty line occurring below
	\eIf{\(\argmax[x_i]{\vcomp_i(x_i)}\) is unique for all \(i\)}{%
		\lForEach{\(i=1,\dotsc,n\)}{\(\hat{x}_i \leftarrow \argmax[x_i]{\vcomp_i(x_i)}\)}
	}{%
		\KwRet{\(\emptyset\)}%
	}
...
% [review] - mention this under implementation instead?
