...
The first update we will consider is the \gls{dp} update (\cref{proc:dp-update,fig:itm-update}).
...
Since the regular \gls{dp} update cannot guarantee optimaly, it is interesting to see if it may be modified to provide such a guarantee.
...
Stronger notions, such as \emph{(full) arc consistency} and \emph{existential arc consistency} \parencite{deGivry05}, are centered around providing very good bound on the problem by projecting costs away from constraints and moving them into the variable components or the nullary constraint, which then immediately provides an easily accessible lower bound on the optimal solution.
...
\Textcite[\pno~85\psq]{deGivry05} present an algorith enforcing this property, and the general structure of this algorithm is very similar to the DP update described above.
...
It may therefore be possible or even desirable to extend notions of stronger arc consistecy to the in-the-middle algorithm.
